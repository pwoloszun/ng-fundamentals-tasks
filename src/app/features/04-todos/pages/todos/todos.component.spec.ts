...
describe('TodosComponent', () => {
  ...
  it('should create todo when todo form data submitted', async () => {
    ...
    const titleControl = component.getByPlaceholderText('Title');
    const descriptionControl = component.getByPlaceholderText('Description');
    const submitBtn = component.getByText('Submit');

    const expectedTitle = 'some new title';
    const expectedDescription = 'my new desc';
    component.type(titleControl, expectedTitle);
    component.type(descriptionControl, expectedDescription);
    component.click(submitBtn);
